...
public class RedisUtil {
    ...
    public <T> T hget(String key, String item, Class<T> c) {
        ...
        if (Objects.nonNull(object)) {
            return JSONObject.parseObject(object.toString(), c);
        }
        return null;
    }
     ...
     * @return
     */
    public DataInfo zGetScoreByRank(String key, long rank, int orderType) {
        Set<ZSetOperations.TypedTuple<Object>> set;
        if (orderType == 1) {
            set = redisTemplate.opsForZSet().rangeWithScores(key, rank, rank);
        } else {
            set = redisTemplate.opsForZSet().reverseRangeWithScores(key, rank, rank);
        }
        if (CollectionUtils.isEmpty(set)) {
            return null;
        }
        for (ZSetOperations.TypedTuple<Object> typedTuple : set) {
            return new DataInfo().setScore(typedTuple.getScore()).setValue(typedTuple.getValue());
        }
        return null;
    }
}